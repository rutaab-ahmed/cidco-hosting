...
export const Edit: React.FC = () => {
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <InputField label="Use (According to File)" name="USE_OF_PLOT_ACCORDING_TO_FILE" value={formData.USE_OF_PLOT_ACCORDING_TO_FILE} onChange={handleChange} />
    ...
    <InputField label="Lease Term (Years)" name="LEASE_TERM_YEARS_" value={formData.LEASE_TERM_YEARS_} onChange={handleChange} />

    <InputField label="Commencement Certificate" name="COMENCEMENT_CERTIFICATE" value={formData.COMENCEMENT_CERTIFICATE} onChange={handleChange} />
    <InputField label="Occupancy Certificate" name="OCCUPANCY_CERTIFICATE" value={formData.OCCUPANCY_CERTIFICATE} onChange={handleChange} />
    ...
    <InputField label="Department Remark" name="Department_Remark" value={formData.Department_Remark} onChange={handleChange} type="textarea" full />
  ...
  <div className="space-y-4">
    {[
      ...
      return (
        <div key={n} className="grid grid-cols-1 md:grid-cols-2 gap-4 border-b pb-4">
          <InputField
            label={`${n}${suffix} Owner Name`}
            name={`NAME_OF_${n}${suffix}_OWNER`}
            value={formData[`NAME_OF_${n}${suffix}_OWNER`]}
            onChange={handleChange}
          />
          <InputField
            label={`${n}${suffix} Transfer Date`}
            name={`_${n}${suffix}_OWNER_TRANSFER_DATE`}
            value={formData[`_${n}${suffix}_OWNER_TRANSFER_DATE`]}
            onChange={handleChange}
          />
        </div>
      );
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <InputField label="Sub Use of Plot" name="SUB_USE_OF_PLOT" value={formData.SUB_USE_OF_PLOT} onChange={handleChange} />
    <InputField label="Plot Status" name="PLOT_STATUS" value={formData.PLOT_STATUS} onChange={handleChange} />
    <InputField label="Survey Remarks" name="SURVEY_REMARKS" value={formData.SURVEY_REMARKS} onChange={handleChange} type="textarea" full />
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    <InputField label="Plot Area for Invoice" name="PLOT_AREA_FOR_INVOICE" value={formData.PLOT_AREA_FOR_INVOICE} onChange={handleChange} />
    <InputField label="Plot Use for Invoice" name="PLOT_USE_FOR_INVOICE" value={formData.PLOT_USE_FOR_INVOICE} onChange={handleChange} />
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <InputField label="PDFs Present" name="PDFS_PRESENT" value={formData.PDFS_PRESENT} onChange={handleChange} />
  </div>

  {/* ================= ACTIONS ================= */}
  <div className="mt-10 flex gap-4 border-t pt-6">
    <button type="submit" disabled={saving} className="bg-indigo-600 text-white px-8 py-3 rounded-lg font-bold">
      Save Changes
    </button>
    <button type="button" onClick={() => navigate(`/details/${id}`)} className="bg-gray-100 px-6 py-3 rounded-lg">
      Cancel
    </button>
  </div>
