...
app.post('/api/users/add', async (req, res) => {
    ...
    try {
        ...
        await pool.query(
            "INSERT INTO users_react (username, password_hash, email, name, role) VALUES ($1, $2, $3, $4, $5)",
...
app.post('/api/users/update-password', async (req, res) => {
    ...
    try {
        ...
        await pool.query("UPDATE users_react SET password_hash = $1 WHERE id = $2", [newHash, userId]);
...
app.post('/api/forgot-password', async (req, res) => {
    ...
    try {
        const users = await pool.query(
            "SELECT id, username, email FROM users_react WHERE username = $1 OR email = $2", 
        ...
        await pool.query(
            "UPDATE users_react SET reset_token = $1, reset_expires = $2 WHERE id = $3",
...
app.post('/api/reset-password', async (req, res) => {
    ...
    try {
        const users = await pool.query(
            "SELECT id FROM users_react WHERE reset_token = $1 AND reset_expires > NOW()",
            [token]
        );
        ...
        await query(
            "UPDATE users_react SET password_hash = $2, reset_token = NULL, reset_expires = NULL WHERE id = $3",
            [newHash, users[0].id]
        );
...
app.get('/api/nodes', async (req, res) => {
    try {
        const rows = await pool.query("SELECT DISTINCT NAME_OF_NODE FROM all_data WHERE NAME_OF_NODE IS NOT NULL AND NAME_OF_NODE != '' ORDER BY NAME_OF_NODE");
...
app.get('/api/sectors', async (req, res) => {
    ...
    try {
        ...
        const rows = await pool.query("SELECT DISTINCT SECTOR_NO_ FROM all_data WHERE NAME_OF_NODE = $1 ORDER BY CAST(SECTOR_NO_ AS UNSIGNED)", [node]);
...
app.get('/api/blocks', async (req, res) => {
    ...
    try {
        // Changed SECTOR_NO to SECTOR_NO_
        const rows = await pool.query("SELECT DISTINCT BLOCK_ROAD_NAME FROM all_data WHERE NAME_OF_NODE = $1 AND SECTOR_NO_ = $2 ORDER BY BLOCK_ROAD_NAME", [node, sector]);
...
app.get('/api/plots', async (req, res) => {
    ...
    try {
        ...
        const rows = await pool.query("SELECT DISTINCT PLOT_NO_ FROM all_data WHERE NAME_OF_NODE = $1 AND SECTOR_NO_ = $2 ORDER BY PLOT_NO_", [node, sector]);
...
app.post('/api/search', async (req, res) => {
    ...
    let sql = `SELECT ID, NAME_OF_NODE, SECTOR_NO_, BLOCK_ROAD_NAME, PLOT_NO_, PLOT_NO_AFTER_SURVEY FROM all_data WHERE NAME_OF_NODE = $1`;
    const params = [node];

    if (sector) { sql += " AND SECTOR_NO_ = $${paramIndex++}"; params.push(sector); }
    if (block) { sql += " AND BLOCK_ROAD_NAME = $${paramIndex++}"; params.push(block); }
    if (plot) { sql += " AND PLOT_NO_ = $${paramIndex++}"; params.push(plot); }
...
app.get('/api/record/:id', async (req, res) => {
    try {
        const rows = await pool.query("SELECT * FROM all_data WHERE ID = $1", [req.params.id]);
