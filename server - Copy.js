...
app.get('/api/record/:id', async (req, res) => {
    try {
        ...
        const imgDir = path.join(uploadsDir, 'images', id);
        let images = [];
        ...
        const has_pdf = fs.existsSync(path.join(uploadsDir, 'pdfs', `${id}.pdf`));
        const has_map = fs.existsSync(path.join(uploadsDir, 'maps', `${id}.pdf`));
